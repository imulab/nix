...
interface OidcAuthorizeRequestRepository {
     ...
     */
    suspend fun save(request: OidcAuthorizeRequest)

    /**
     * Retrieves the request by its id. Implementation should return null if such request is not found.
     */
    suspend fun get(requestId: String): OidcAuthorizeRequest?
 ...
 * a time-to-live configuration. It is not thread-safe either.
 */
class MemoryOidcAuthorizeRequestRepository : OidcAuthorizeRequestRepository {
    ...
    override suspend fun save(request: OidcAuthorizeRequest) {
        db[request.id] = request
    }

    override suspend fun get(requestId: String): OidcAuthorizeRequest? = db[requestId]
